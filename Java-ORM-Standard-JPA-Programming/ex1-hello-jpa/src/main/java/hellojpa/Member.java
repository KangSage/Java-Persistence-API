...
public class Member {
  ...
  /**
   * Column 애노테이션에 사용할 수 있는 옵션들<BR>
   * insertable, updatable : JPA에서 insert, update를 막을 수 있다. 기본 값은 true<BR>
   * nullable: false일 경우 not null 제약 조건. 기본 값은 true<BR>
   * unique: 유니크 제약 조건 여부<BR>
   *         - constraint Key 이름을 알아보기 힘들게 되므로 @Table의 uniqueConstraints 옵션을
   *         주로 사용한다.<BR>
   * length: String 타입 필드의 길이를 정의<BR>
   * columnDefinition: DB 컬럼 정보를 직접 줄 수 있다.<BR>
   * precision: 소수점을 포함한 전체 자릿수 BigDecimal, BigInteger 타입에서 사용한다.<BR>
   * scale: 소수의 자릿수. double, float에는 적용되지 않고 정말 정밀한 자릿수에만 사용 가능.<BR>
   */
